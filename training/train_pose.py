...
from training.dataset import initialize_dataflow, build_batch_dataflow
from training.dataflow import COCODatasetPaths
...
max_iter = 1 # epoch just to initally train for one epoch
...
if __name__ == '__main__':
    ...
    # prepare generators
    base_dir = os.getcwd()
    annot_path_train = os.path.join(base_dir, 'datasets/annotations/person_keypoints_train2017.json')
    print(annot_path_train)
    img_dir_train = os.path.abspath(os.path.join(base_dir, 'datasets/train2017/'))
    annot_path_val = os.path.join(base_dir, 'datasets/annotations/person_keypoints_val2017.json')
    img_dir_val = os.path.abspath(os.path.join(base_dir, 'datasets/val2017/'))


    # get dataflow of samples from training set and validation set (we use validation set for training as well)

    coco_data_train = COCODatasetPaths(
        annot_path=annot_path_train,
        image_directory=img_dir_train
    )
    coco_data_val = COCODatasetPaths(
        annot_path=annot_path_val,
        image_directory=img_dir_val
    )
    df = initialize_dataflow([coco_data_train, coco_data_val])
    train_samples = df.size()

    # get generator of batches

    batch_df = build_batch_dataflow(df, batch_size)
